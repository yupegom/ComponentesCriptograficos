...
define
   proc {ICodificadorRSA PuertoOpMatematicas Flujo PuertoCodificador}
      {Componente.nuevoPuertoReq
       proc{$Mensaje} CodificadorRSA = {New CodificadorRSARealize.codificador init} Clave
       in
	  case Mensaje of codificar(TextoACodificar N E TamanoBloque ?TextoCodificado) then
	     try
			...
			TextoCodificado = {CodificadorRSA codificar(TextoACodificar Clave PuertoOpMatematicas RealizarOperacion TamanoBloque $)}
	  ...
	  [] decodificar(TextoADecodificar N D TamanoBloque ?TextoDecodificado) then
	     try
			...
			TextoDecodificado = {CodificadorRSA decodificar(TextoADecodificar Clave PuertoOpMatematicas RealizarOperacion TamanoBloque $)}
